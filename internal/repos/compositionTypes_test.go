...
var _ = Describe("repo:CompositionTypes", func() {
	...
	Context("FindOrCreate", func() {
		It("should successfully create a composition type", func() {
			at, err := repo.FindOrCreate(ctx, types.CreateCompositionType{Name: "Regular Infantry", GameID: gm.ID})
			Expect(err).To(BeNil())
			Expect(at.ID).NotTo(HaveLen(0))

			at2, err := repo.FindOrCreate(ctx, types.CreateCompositionType{Name: "Regular Infantry", GameID: gm.ID})
			Expect(err).To(BeNil())
			Expect(at2.ID).NotTo(HaveLen(0))
			Expect(at.ID).To(Equal(at2.ID))

			at3, err := repo.FindOrCreate(ctx, types.CreateCompositionType{Name: "Heavy Cavalry (named character)", GameID: gm.ID})
			Expect(err).To(BeNil())
	...
	Context("Find", func() {
		BeforeEach(func() {
			...
			at2, err := repo.FindOrCreate(ctx, types.CreateCompositionType{Name: "Heavy Cavalry (named character)", GameID: gm.ID})
			Expect(err).To(BeNil())
			Expect(at2.ID).NotTo(HaveLen(0))

			at3, err := repo.FindOrCreate(ctx, types.CreateCompositionType{Name: "War Beasts", GameID: gm.ID})
			Expect(err).To(BeNil())
			Expect(at3.ID).NotTo(HaveLen(0))
		})
